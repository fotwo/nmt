...
wc -l casict2015_ch.chars.ipa.txt
wc -l casict2015_ch.chars.syl.txt
# 1833959 -30000 = 1803959


tail -n 1803959 casict2015_ch.chars.ipa.txt | cat - ../ipa2syl/train.ipa > train.ipa
tail -n 1803959 casict2015_ch.chars.syl.txt | cat - ../ipa2syl/train.syl > train.syl

tail -n 30000 casict2015_ch.chars.ipa.txt | cat - ../ipa2syl/dev.ipa > dev.ipa
tail -n 30000 casict2015_ch.chars.syl.txt | cat - ../ipa2syl/dev.syl > dev.syl
...
for f in ${OUTPUT_DIR}/*.syl; do
  fbase=${f%.*}
  echo "Cleaning ${fbase}..."
  ${OUTPUT_DIR}/mosesdecoder/scripts/training/clean-corpus-n.perl $fbase ipa syl "${fbase}.clean" 1 160
done
...
CUDA_VISIBLE_DEVICES=2 python -m nmt.nmt \
    ...
    --hparams_path=nmt/standard_hparams/casict2015.json
######
######
